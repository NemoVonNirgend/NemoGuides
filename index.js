...
async function onChatChanged() {
    ...
    if (isNewChat) {
        ...
        // Auto-run prompt advisor on character/chat change if enabled
        const settings = extension_settings[EXTENSION_NAME];
        if (settings?.autoAdvisor) {
            // Delay slightly to let the chat fully load
            setTimeout(() => {
...
async function onFirstMessage() {
    ...
    if (userMessages === 1) {
        console.log(`${LOG_PREFIX} First message in chat — wiping tracker entries for fresh start.`);
        await clearAllTrackers();

        // Initialize DM notes scratchpad
        initDMNotes().catch(err => {
            console.error(`${LOG_PREFIX} DM notes initialization failed:`, err);
        });

        // Note: Rule Setup, Scene Assessment, and DM Notes initialization are handled
        // via tool calls — the system instruction tells the AI to call all three on a
        // fresh chat. This avoids race conditions with programmatic auto-generation.
    }
}
...
async function init() {
    ...
    // Bind settings events and populate UI
    bindSettingsEvents();
    updateSettingsUI();
